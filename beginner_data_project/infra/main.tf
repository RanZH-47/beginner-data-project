...
resource "aws_iam_instance_profile" "sde_ec2_iam_instance_profile" {
...
resource "aws_security_group" "sde_security_group" {
  ...
  description = "Security Group that allows inbound SCP and outbound 8080 Airflow connections"
...
resource "aws_instance" "sde_ec2" {
  ...
  security_groups = [aws_security_group.sde_security_group.arn]

  iam_instance_profile = aws_iam_instance_profile.sde_ec2_iam_instance_profile.id
...
resource "aws_budgets_budget" "my_cost" {
  budget_type  = "COST"
  limit_amount = "10"
  limit_unit   = "USD"
  time_unit    = "MONTHLY"
